...
from executorch.backends.nxp.tests.models import AddTensorModule, AddTensorConvModule, AddTensorOneInputModule
...
def test_add_tensor_w_conv_quant_conversion(mocker, input_shape):
    ...
    convert_run_compare(exported_program, input_data, tflite_input_preprocess=ToChannelLastPreprocess(),
                        tfl_model=tflite_flatbuffers_model, tflite_output_preprocess=ToChannelFirstPreprocess())


@pytest.mark.parametrize("x_input_shape, y_input_shape", [
...
def test_add_tensor_broadcasting_unsupported_quant_conversion(x_input_shape, y_input_shape):
    model = AddTensorModule()

    # Run conversion
    edge_program = to_quantized_edge_program(model, [x_input_shape, y_input_shape]).exported_program()
