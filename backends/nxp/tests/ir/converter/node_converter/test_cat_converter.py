...
@pytest.mark.parametrize("rank, num_inputs, dim", [
...
def test_cat__same_shapes(dim, num_inputs, rank, mocker):
    ...
    quantized_program = to_quantized_edge_program(CatModule(dim), [input_shape] * num_inputs).exported_program()
...
@pytest.mark.parametrize("num_inputs", [
    2, 5
])
...
def test_cat__unsupported_dim__imxrt700(dim, num_inputs):
    ...
    quantized_program = to_quantized_edge_program(CatModule(dim), [input_shape] * num_inputs,
                                                  target='imxrt700').exported_program()
...
def test_cat__different_shapes(dim, num_inputs, rank, mocker):
    ...
    quantized_program = to_quantized_edge_program(CatModule(dim), input_shapes).exported_program()
...
@pytest.mark.parametrize("num_inputs", [
    2, 5
], ids=lambda num_inputs: f'num_inputs = {num_inputs}')
def test_cat__channels_first__different_shapes(dim, num_inputs, mocker):
    ...
    for i in range(num_inputs):
        tmp_shape = list(input_shape)
        tmp_shape[dim] = 8 * (i + 1)  # Neutron only supports channels that are multiples of 8 (on RT700).
        input_shapes.append(tuple(tmp_shape))

    converter_spy = mocker.spy(EdgeProgramToIRConverter, "convert_program")

    channels = sum(shape[1] for shape in input_shapes) if dim in [1, -3] else input_shape[1]
    quantized_program = to_quantized_edge_program(
        CatConvModule(dim, channels),
        input_shapes
    ).exported_program()
...
def test_cat__different_shapes__unsupported_channels__imxrt700():
    ...
    quantized_program = to_quantized_edge_program(CatModule(dim), input_shapes, target='imxrt700').exported_program()
