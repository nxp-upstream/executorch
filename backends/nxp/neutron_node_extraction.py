...
def extract_artifacts_from_neutron_node(tflite_flatbuffer_or_path: bytes | str) -> NeutronNodeArtifacts:
    ...
    for tensor_idx in node_inputs:
        ...
        assert which_graph_input.size == 1, 'Mismatch between Neutron Node inputs and graph inputs.'
        input_indices.append(which_graph_input[0])
        input_names.append(sub_graph.Tensors(graph_inputs[which_graph_input[0]]).Name())

    assert neutron_node.OutputsLength() >= 2, \
    ...
    for tensor_idx in node_outputs:
        ...
        assert which_graph_output.size == 1, 'Mismatch between Neutron Node outputs and graph outputs.'
        output_indices.append(which_graph_output[0])
        output_names.append(sub_graph.Tensors(graph_outputs[which_graph_output[0]]).Name())

    return NeutronNodeArtifacts(input_names, input_indices, output_names, output_indices, microcode, weights, kernels)
